...
#ifndef SMPPOUTPUTTERBASE_HPP
#define SMPPOUTPUTTERBASE_HPP
...
namespace anthill {
	namespace smpp {
		 ...
		 * A SMPP client 
		 */
		class ANTHILLSMPP_DLLPRFX SMPPOutputterBase : public ISMPPOutputter{

		public:
			SMPPOutputterBase(void);
			virtual ~SMPPOutputterBase();

		public:
			/**
			 * Methods to return name based on code
			 */
			std::string getCommandName(const unsigned nCommandId);
			std::string getErrorName(const unsigned nErrorCode);
			std::string getNetworkErrorName(const unsigned char nNetworkErrorCode);
			std::string getMessageStateName(const unsigned char nMessageState);

			/**
			 * Methods to return information from esm class
			 */
			std::string getMessagingMode(const unsigned char nEsmClass);
			std::string getMessageType(const unsigned char nEsmClass);
			std::string getMessageFeatures(const unsigned char nEsmClass);
			/**
			 * Methods to return area format name
			 */
			std::string getAreaFormatName(const unsigned char nAreaFormat);
			/**
		...
		protected:
			SMPPEncoder m_oEncoder;
		};
	}
}

#endif // SMPPOUTPUTTERBASE_HPP
